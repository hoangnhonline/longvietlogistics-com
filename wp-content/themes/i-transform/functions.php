...
function itransform_excerpt_more($more) {
       global $post;
       $text_read_more = (qtrans_getLanguage()=='en') ? "Read more" : "Xem thêm";
	return '<a class="moretag" href="'. get_permalink($post->ID) . '">'. __( $text_read_more.'...', 'itransform' ). '</a>';
}
add_filter('excerpt_more', 'itransform_excerpt_more');


if(!function_exists('cptui_register_my_cpt_freight')){
	
		add_action('init', 'cptui_register_my_cpt_freight');
			function cptui_register_my_cpt_freight() {
			register_post_type('freight', array(
			'label' => 'Freights',
			'description' => '',
			'public' => true,
			'show_ui' => true,
			...
			'map_meta_cap' => true,
			'hierarchical' => false,
			'rewrite' => array('slug' => 'freight', 'with_front' => true),
			'query_var' => true,
			...
			'labels' => array (
			  ...
			  'menu_name' => 'Freights',
			  'add_new' => 'Add Freight',
			  'add_new_item' => 'Add New Freight',
			  'edit' => 'Edit',
			  'edit_item' => 'Edit Freight',
			  'new_item' => 'New Freight',
			  'view' => 'View Freight',
			  'view_item' => 'View Freight',
			  'search_items' => 'Search Freights',
			  'not_found' => 'No Freights Found',
			  'not_found_in_trash' => 'No Freights Found in Trash',
			  'parent' => 'Parent Freight',
			)
			) ); }
